...
function HistoryCard({ item, onDelete }) {
  ...
  return (
    <div className="card p-4 flex flex-col gap-3 animate-fade-up hover:shadow-card-hover transition-all">
      {/* City + Date */}
      <div className="flex items-start justify-between">
        <div>
          ...
          <h3 className="text-lg font-bold text-ink capitalize">{item.city}</h3>
        ...
        <button
          ...
          className="p-2 rounded-lg text-muted hover:text-danger hover:bg-red-500/10
                     transition-all"
        >
          <Trash2 size={14} />
        </button>
      ...
      {item.weather && (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
          ...
          <div>
            <p className="text-xl font-bold text-white">{item.weather.temp}°C</p>
        ...
        <div>
          ...
          <ul className="space-y-1">
            {item.newsHeadlines.slice(0, 2).map((h, i) => (
              <li key={i} className="text-xs text-white/70 leading-snug line-clamp-1 pl-2
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-8 animate-fade-up">
          <div className="w-10 h-10 rounded-xl bg-blue-600/10 border border-blue-500/15
